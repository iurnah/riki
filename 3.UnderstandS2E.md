...
Let's see a very simple exaple of signal and callback from the [tutorial of
sigc++][alienDetector].  We want to write a application that inform users when an alien landed on car parking. We use libsigc++ to implement this application. There is a `AlienDetector` class needed. There is a signal defined in it. 
...
{
	cout << "sigc::ptr_fun# alerter() \n" << "There are aliens in the " << where << "!" << endl;
}
