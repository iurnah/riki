...
It first connect to `onStateFork`, `onTimer`, and `onProcessFork` signals. when the `onStateFork` is emitted (from other components, actually in S2EExecutor.cpp), the `ExecutionTracer::onFork` callback will  write a item to ExecutionTrace.dat. Which can be find in the trace results by searching keyworkds: __Forked at__. Accept this, the ExecutionTracer didn't do anything much more, it just provide a API for other plugins to use. 
...
It base on principles that a TB_INTERRUPT at `onTranslateBlockEnd` will indicate a interrupt start signal, while `JT_IRET` at `onTranslateBlockStart` indicate a intrrupt return signal. The two call back function conneted to those signals are `InterruptMonitor::onInterrupt` and `InterruptMonitor::onInterruptReturn`. 
...
The `m_newCallDesriptor` is keep per state call descriptors, each time calling `X86FunctionMonitorState::slotCall`, it insert all its element to `m_callDescriptors`, which have the globle call signal descriptor. Next it test whether there are nothing in the `m_callDescriptors`, if false, __it return all call descriptors in `m_callDescriptors` that have the same key.__ This key is the address of the funcion in memory. __What does this mean is it return all the calls in the form of `CallDescriptor` for a specific function from a specific memory address.__ the it iterate through the "range" of different descriptors and check the pid of those call descriptors to search the one that we interested (current state pid).(see how multimap and the CallDescriptorsMap ADT implemented to understand this) If found, the signal is emitted for the specific call descriptor. The connection for this call signal is in the client program, and it is not straightfoward connection. Refer to the `LibararyCallMonitor.cpp` for how the client connect to this signal.

We can see in the client program, it first need to call the `X86FunctionMonitor::getCallSignal()` to obtain a `callSignal`. This function then call the `X86FunctionMonitorState::getCallSignal()` to query the `m_callDescriptors` for the specific cr3 (pid). if found, it return the registered `callSignal`. if not found, it insert the call signal to the `m_newCallDescriptors` and return the inserted signal. Keep in mind that the `m_newCallDescriptrs` is a per state `CallDescriptorsMap`, it will be insert to the globale call descriptor data structure `m_callDescriptors`.
