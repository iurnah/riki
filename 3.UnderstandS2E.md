...
- [OSMonitor.h](#OS Monitor.h) _define signals such as `onModuleLoad`, `onProcessUnload`, and member functions such as `getImports()`, `getCurrentStack()`._
...
Implement the member functions including `loadModule()` and `opLoadModule()`. In fact `loadModule()` is called from anothe member function `RawMonitor::onTranslateInstructionStart()`.
```cpp
