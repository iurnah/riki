...
- [Execution State](#ExecutionState) list of the globle execution state objects.
...
- [RawMonitor.cpp](#RawMonitor.cpp) _emit signal after load module successfully in `loadModule` and `opLoadModule`._
...
- [ModuleExecutionDetector.cpp](#ModuleExecutionDetector.cpp) function to read configured module info, define `moduleLoadListener`, `onTranslationBlockStart` member functions. `class ModuleTransitionState`.
- [init_env.c](#init_env.c) guest tool.
- [CodeSelector.h](#CodeSelector.h)
- [CodeSelector.cpp](#CodeSelector.cpp) _use onPrivilegeChange and onPageDirectoryChange signal to enable and disable forking, ensuring the right code has been selected to do SE._
- [TranslationBlockTracer.h](#TranslationBlockTracer.h)
- [TranslationBlockTracer.cpp](#TranslationBlockTracer.cpp) _records the execution of all translation blocks in the modules of interest. Recorded information includes that start address of each block as well as the contents of CPU registers before and after the execution._
...
- [ModuleTracer.h](#ModuleTracer.h) _`class ModuleTracerState`, and `addModule`, `getCurrentModule` member functions._
- [ModuleTracer.cpp](#ModuleTracer.cpp) _also define the member function `moduleLoadListener`, find out the difference between it and `ModuleExecutionDetector.cpp`_
- [InterruptMonitor.h](#InterruptMonitor.h)
- [InterruptMonitor.cpp](#InterruptMonitor.cpp)
- [LinuxSyscallMonitor.h](#LinuxSyscallMonitor.h)
- [LinuxSyscallMonitor.cpp](#LinuxSyscallMonitor.cpp)
- [LibraryCallMonitor.h](#LibraryCallMonitor.h)
- [LibraryCallMonitor.cpp](#LibraryCallMonitor.cpp)
- [FunctionCallMonitor.h](#FunctionCallMonitor.h)
- [FunctionCallMonitor.cpp](#FunctionCallMonitor.cpp)
- [StackMonitor.h](# StackMonitor.h)
- [StackMonitor.cpp](# StackMonitor.cpp)
...
- [S2E.h](#S2E.h)
- [S2E.cpp](#S2E.cpp)
- [S2EExecutor.h](#S2EExecutor.h)
- [S2EExecutor.cpp](#S2EExecutor.cpp)
- [S2EExecutionState.h](#S2EExecutionState.h)
- [S2EExecutionState.cpp](#S2EExecutionState.cpp)
- [Plugin.h](#Plugin.h)
...
For 2. You need to understand how signal works and how the callback parameters is passed. Let's see a very simple example of signal and callback from the [tutorial of sigc++][alienDetector]
...
There need a client to run this plugin and take the emit of signal by connecting it. Any Plugin or program that has `onPriodicEvents.connect()` will work as the client. Furthermore, we can see this `InstructionTracker` Plugin as the client of the `CorePlugin` because it connects some of the signal declared in `CorePlugin`.

Here the signal emition is different from most of the CorePlugin event emition. Here the emition is in a callback function. For S2E CorePlugins, they are emitted in a function that called from QEMU code. (target-i386/translate.c)

The emition and connection of a signal can be used flexibly. The emition is like the trigger of passing parameters by the signal. The connection is connecting the signal with the callback function, which take the passed parameters as its function parameters and do something in the defination of the callback function.
...
About 248 line of code, declare two data structure `struct S2EShared` for synch among multiple instances of S2E, and `class S2E` for initialize . The `struct S2EShared` is:
...
```
The `S2E` maintain some variables and data structures for QUME. At this moment, I do not get into QEMU and LLVM. `S2E` member functions include: 
- `getConfig()`, 
- `getPlugin()`, 
- `getCorePlugin()`, 
- `getOutputDirectory()`, 
...
Most of those member functions are for QEMU to call. Generally, it is the S2E API for QEMU. The Plugins are talk to QEMU through S2E class.
...
```cpp
        ...
        ...
```
The process run as a children process, so have todo some initialization in the beginning. Then there are more QEMU functions called.
...
```
It basically return an initialized object to the caller.

On s2e_close, it first print a stack trace by calling the function ` print_stacktrace()`, which is defined at the beginning of this file. Read the source of this function if you have time. I might be very valuable in implement this technique.

<a name="S2EExecutor.h"/>
## S2EExecutor.h

<a name="S2EExecutor.cpp"/>
## S2EExecutor.cpp

<a name="S2EExecutionState.h"/>
## S2EExecutorState.h

<a name="S2EExecutionState.cpp"/>
## S2EExecutorState.cpp
...
<a name="ExecutionState" />
## ExecutionState

- g_s2e ./S2E.cpp: S2E* g_s2e = NULL;
- g_s2e_state ./s2e_qemu.h: extern struct S2EExecutionState* g_s2e_state;
	extern struct S2EExecutionState* g_s2e_state;
- s2e()
	Plugin.h: S2E* s2e() { return m_s2e; }
- state in BaseInstruction.cpp (S2EExecutionState *state)
- struct S2ETranslatioinBlock
```cpp
// S2EExecutor.h
...
```
Those functions are instrumented into the proper place of QEMU source code. When they are called, they essentially pass the parameters to the callback function, provided that the callback and the signal has been connected.


<a name="Example: onCustomInsruction" />
## Example: onCustomInsruction
**important**
How S2E customized code work related to the term `onCustomInstruction`, `handleBuiltInOps`. This is the big picture how it works:

1. Insert `s2e_make_symbolic()`, `s2e_enable_fork()` is defined to be inline assembly inserted to the binary code. (while using init_env.so shared library, it employ other techniques to do this.)
2. The assembly code inserted to binary has S2E customized opcode, When QEMU translate code. This special opcode will be handled by a specific function `s2e_tcg_emit_custom_instructions(g_s2e, arg)`. 
3. This function `s2e_tcg_emit_custom_instructions(g_s2e, arg)` is implemented in the Coreplugin.cpp, in which it will be translated into intermediate representation code. More specifically, it based on `tcg_gen_helpN()` helper function to write the opcode functionality as an C funcion,but in tcg IR format. 
4. On executing, the particular IR function will translate to the host platform instructions and execute. 
5. The function will emit signals for Plugins to receive. 
6. The effect of s2e customized opcode (e.g. s2e_make_symbolic in BaseInstruction) is implemented through some member functions. 
7. Those member function call KLEE/LLVM's api throught s2e's execution state, (i.e. `state->enableForking()`) to either enable the forking or make the variable symbolic. 
8. KLEE's execution engine will take care of the execution based on what the opcode instructed it to do
...
Not all of the plugin need these "onCustomInstruction" events, this events only related to some of the plugins that involving modify the tcg code with s2e opcode. When those opcodes are disasembled, they are not given to CPU to execute, instead, they are handled by s2e opcode handler such as discussed above.

The following plugins are have `onCustomInstruction` events related to them.
1. BaseInstructions
2. CodeSelector
3. ModuelExecutionTracer
4. ExecutionTracer
	
###1. BaseInstructions.cpp
```cpp
...
Note it also have `printExpression(state)`, `printMemory(state)`, and `printMessage(state)`. 
...
```
Implement the member functions including `loadModule()` and `opLoadModule()`. In fact `loadModule()` is called from another member function `RawMonitor::onTranslateInstructionStart()`. the reason of doing this is because it automatically detect the module has been loaded by the onTranslateInstructionStart events. 
```cpp
...
```
In the `RawMonitor::initialize()` function, the `onTranslateInstructionStart` signal is connected, this signal is emitted upon code generation for each instruction Translation. In order to detect the module load, we can emit the `onModuleLoad.emit()` signal once the first code generation instruction will start (onTranslateInstructionStart), we then immediately disconnect the signal, future emittion of `onTranslateInstructionStart` will not take any effect.

Thusly, we can summarize what the RawMonitor.cpp is doing. It basically take the very first `onTranslateInstructionStart` signal to signaling the module is loaded. It then print the configured module, this `CfgList` should be automatically checked before.
...
It has the following function definitions, we are interested in what data structure it used to maintain all the module information.
...
```cpp
    ...
    */
	...
	onModuleLoad;
```
~Pay attention to the comment about `onModuleTranslateBlockStart`, it doesn't match the meaning of `onModuleTranslateBlockStart`. When you want to use those signals, you have to track back to the signal emit spot to identify which on is true~

Above is wrong, emmitted at both beginning and end are make sense, because we are not talking about same module.
...
> A module is a piece of code defined by a name. Currently the pieces of code are derived from the actual executable files reported by the OS Monitor.
...
```
The member function `void ModuleExecutionDetector::initialize()` is does what it said. It read the configure list in the .lua file and add each of those configured module to the set `m_ConfigureModulesName`. It check the uniqueness of the ids and moduleNames before adding.
...
Other member functions that are very interesting are:
```cpp
...
It also connect the `ModuleExecutionDetector::onExcution` callback function with the current state instruction execution signal. This signal is very important because we want to monitor syscall and library from now on. More specifically, we can connect this signal to a member function `SyscallMonitor::onBlockExecution()`, in which we enable the syscall interceptions until next `onModuleTranslateStart` emit signal, which basically mean the monitored block is finished execution. The process is start all over. We check whether next block is the block we are interested, upon receive the `onTranslateBlockStart` signal, we invoke the onExecution syscall, do the same things, which is system call monitor and library call monitor.

<a name="init_env.c" />
## init_env.c

In this guest library implementation, it actually talk to (doing part of their job) RawMonitor and ModuleExecutionDetector Plugins. It call `s2e_rawmon_loadmodule2()` to register modules use a opcode. This is why some of the member function defined with the `op` prefix. And most of them are executed upon the `onCustomInstruction` signal emittion, __which is emited every time when custom opcode is detected.__

Also keep an eye on the inline assembly function call `s2e_moduleexec_add_module(base, base, 0)`; It programmatically add a new configuration entry to the ModuleExecutionDetector plugin.

Here is some of other inline function called from this file which is important.
```
line#
...
Please also pay attention to the technique used in this file to intercept a library function. Google `dlsym()` and `RTLD_NEXT` keywords.

The init_env.c has refered to the following plugin opcode,
1. RawMonitor: `s2e_rawmon_loadmodule2()` (put the &cfg in register ECX)
2. ModuleExecutionDetector: `s2e_moduleexec_add_module()` (put the module namem in the eax, module id in the ecx, and module size in the edx)
3. CodeSelector: `Code_Selector_
...
<a name="TranslationBlockTracer.h" />
## TranslationBlockTracer.h

<a name="TranslationBlockTracer.cpp" />
## TranslationBlockTracer.cpp

> The TranslationBlockTracer plugin records the execution of all translation blocks in the modules of interest. Recorded information includes that start address of each block as well as the contents of CPU registers before and after the execution.

It define the following member functions:
```cpp
    initialize()
    enableTracing() 
    disableTracing()
...
The `trace()` is the doing the job of reading CPU registers and write to ExecutionTracer. I should focus on this function to understand the method to read register values.
```cpp
...
```
Generally the plugin works as following: (1) call `initialize()`, in which based on the configure value read to enableTracing immediately or connect the `onCustomInstruction` signal to listen TranslationBlockTracer opcode. (2) when the `enableTracing()` is called, it connect the signal `onModuleTranslateBlockStart` and `onModuleTranlateBlockEnd` to their corresponding member function. 
In the member function, what it really does is to connect the current state signals (emitted on instruction execution). Because this signal is emitted in each instruction execution, when it connected, the signal is immediately fired. What is said here is the `onModuleTranslateBlockStart` indicate event `onExecutionBlockStart` is going to happen after. In the `onExecutionBlockStar` callback function, it call `trace()` to read register values and write the `ExecutionTraceTb` item to `ExecutionTracer` plugin by the `writeDate()` API.

When calling `trace`, the third parameter should specify what type of trace it is. Either it is TRACE_TB_START or TRACE_TB_END, or we can define the third type: __TRACE_TB_IN__. because we want to have the register value during the block translation.

Since we don't configure the manualTrigger, we don't discuss the functions related to `onCustomInstructions`. 
...
> The ExecutionTracer plugin is the main tracing plugin. This plugin saves a binary trace file in the `ExecutionTracer.dat` file. This file is composed of generic trace items. Each item can have an arbitrary format, determined by the various client tracing plugins. The client plugins call the ExecutionTracer's API to write trace items.
 
> By default, ExecutionTracer records the program counters where fork occurs. This allows offline analysis tools to rebuild the execution tree and provide per-path analyses.

It first connect to `onStateFork`, `onTimer`, and `onProcessFork` signals. when the `onStateFork` is emitted (from other components, actually in S2EExecutor.cpp), the `ExecutionTracer::onFork` callback will  write a item to ExecutionTrace.dat. Which can be find in the trace results by searching keywords: __Forked at__. Accept this, the ExecutionTracer didn't do anything much more, it just provide a API for other plugins to use. 

`onProcessFork` signal is also emitted in the file `S2EExecutor.cpp`. It will create new trace file for the new process. We don't use it much.
...
It is basically reimplement the signals in the `ModuleExeuctionDetection` plugin. The reason it exist is because it trace and write the module load and unload information to the ExecutionTracer.dat file and may help in future file parsing. We are not necessarily going to use it in our project.

<a name="InterruptMonitor.h" />
## InterruptMonitor.h

<a name="InterruptMonitor.cpp" />
## InterruptMonitor.cpp

It base on principles that a TB_INTERRUPT at `onTranslateBlockEnd` will indicate a interrupt start signal, while `JT_IRET` at `onTranslateBlockStart` indicate a interrupt return signal. The two call back function connected to those signals are `InterruptMonitor::onInterrupt` and `InterruptMonitor::onInterruptReturn`. 

`InterruptMonitor::onInterrupt` will read the CPU register value to decide what interrupt it is and generate a signal related to it, and the same interrupt return signal will be called once the interrupt returns. (How the signal map is managed isn't clear to me) __TODO:__ understand the following code gist:
```cpp
	...
	s2e()->getDebugStream() << "Received interrupt 0x" << std::hex << intNum  \
				<< " at 0x" << pc << std::dec << std::endl;
```
~__Where is this emitted signal has been connected?__~
__Answer: The signal

<a name="LinuxSyscallMonitor.h" />
## LinuxSyscallMonitor.h

<a name="LinuxSyscallMonitor.cpp" />
## LinuxSyscallMonitor.cpp

The `LinuxSyscallMonitor::onTranslateBlockEnd` import the `InterruptMonitor` plugins in the initialized steps and then take care of three different syscall implementations. It use interruptSignal and the translation block type `TB_SYSENTER` and `TB_SYSEXIT` to enable the detection of a system call.
```cpp
...
```
Upon the `LinuxSyscallMonitor::onInt80` called, it first take care of the 0x80 interrupt opcode by calling the emitSyscallSignal(state, pc, SYSCALL_INT, signal); What this `emitSyscallSignal` does is it first read CR3 (The address of the page directory of current virtual address space). Then it read the EAX (contain the syscall number). Following this is to read the parameters one by one from the general purpose register of x86. The first seven contain the parameter of the called system call. Then it look for whether the syscall number is in the `SyscallSignal` map, it true, it will emit signal for this particular syscall to do the specific thing by the callback functions. This is quite parallel to the interrupt signal handling, emit in the plugins, signal connection and callback definition is in the client plugin. (For interruptMonitor.cpp, the client is `LinuxSyscallMonitor.cpp`) Otherwise it will emit signals for all the system calls.

We can also understand in this way: the `LinuxSyscallMonitor` plugin export the monitored system call, it import through a function connection combined with client defined callback function. __This is the REWARD  like implementation that I should work on__

<a name="LibraryCallMonitor.h" />
## LibraryCallMonitor.h

<a name="LibraryCallMonitor.cpp" />
## LibraryCallMonitor.cpp

<a name="x86FunctionCallMonitor.h" />
## FunctionCallMonitor.h

<a name="x86FunctionCallMonitor.cpp" />
## FunctionCallMonitor.cpp

In the initialize routine, it connect two `CorePlugin` events: `onTranslateBlockEnd` and `onTranlateJumpStart`, the first one connect to `X86FunctionMonitor::slotTranslateBlockEnd` slot function and the second one connect to `X86FunctionMonitor::slotTranslateJumpStart` slot function. where is in each slot function, it check the type of the translated blocks and connect to the dynamic created signal, which is evoked on each instruction execution. This dynamic signal has been connected to the `slotCall` member function. In the `slotCall` member function, it simply use the `DECLARE_PLUGINSTATE` to return the `plgState` and call `plgState's` own member function slotCall. This is to keep a per state function call signals. The `X86FunctionMonitorState::slotCall` function is shown below. 
```cpp
...
```
This member function is to manage the signals for client plugin to connect and emit each call signal and return signal properly. __The `CallSignals` are connected in the client plugin!!!__

The `m_newCallDesriptor` is keep per state call descriptors, each time calling `X86FunctionMonitorState::slotCall`, it insert all its elements to `m_callDescriptors`, which keep the global call signal descriptors. Next it test whether there are nothing in the `m_callDescriptors`, if false, __it return all call descriptors in `m_callDescriptors` that have the same key.__ This key is the address of the function in memory. __What does this mean is it return all the calls in the form of `CallDescriptor` for a specific function from a specific memory address.__ the it iterate through the "range" of different descriptors and check the pid of those call descriptors to search the one that we interested (current state pid).(see how multimap and the CallDescriptorsMap ADT implemented to understand this) If found, the signal is emitted for the specific call descriptor. The connection for this call signal is in the client program, and it is not straightforward connection. Refer to the `LibararyCallMonitor.cpp` for how the client connect to this signal.

We can see in the client program, it first need to call the `X86FunctionMonitor::getCallSignal()` to obtain a `callSignal`. This function then call the `X86FunctionMonitorState::getCallSignal()` to query the `m_callDescriptors` for the specific cr3 (pid). if found, it return the registered `callSignal`. if not found, it insert the call signal to the `m_newCallDescriptors` and return the inserted signal. Keep in mind that the `m_newCallDescriptrs` is a per state `CallDescriptorsMap`, it will be inserted to the global call descriptor data structure `m_callDescriptors`.

The return signal handling is quite parallel to call signal handling. But we still want to look at how it implemented in the function `void X86FunctionMonitorState::slotRet`. __The current plugins doesn't use many `registerReturnSignal` functions and `slotRet`. but it is quite simple.__

<a name="StackMonitor.h" />
## StackMonitor.cpp

<a name="StackMonitor.cpp" />
## StackMonitor.cpp

This is a plugin that to be finished in future. I will study it and finish it in future.
